...
export async function generateMetadata() {
  return {
    title: 'Nakliyeciler Sitesi | Kamyon ve Tır Nakliye, 81 İl',
    description:
      '81 ilde kamyon nakliye, tır nakliye ve kamyon garajı. Km bazlı fiyat tablosu ve hesaplama aracıyla Adana çıkışlı yük taşıma ücretinizi hemen görün.',
    alternates: { canonical: `${SITE_URL}/nakliyeciler-sitesi` },
  }
}

export default async function NakliyecilerDizini() {
  ...
  return (
    <>
      ...
      <main id="main-content">
        <section className="py-16 md:py-20" style={{ background: 'linear-gradient(135deg, #0b3b73 0%, #0561e0 100%)' }}>
          <div className="container mx-auto px-4">
            <nav aria-label="Sayfa yolu" className="mb-4 flex items-center gap-2 text-sm text-white">
              ...
              <span className="font-medium text-white">Nakliyeciler Sitesi</span>
            </nav>
            <h1 className="text-3xl font-bold tracking-tight text-white md:text-5xl">İllere Göre Nakliyeciler Sitesi</h1>
            <p className="mt-4 max-w-3xl leading-relaxed text-white">
              Ev eşyası değil yük taşıtacaksanız doğru yerdesiniz. 81 ilin tamamı için kamyon nakliye, tır nakliye ve kamyon garajı hizmetlerimizi, km bazlı fiyat tablosunu ve tahmini ücreti anında gösteren hesaplama aracını il sayfalarında bulabilirsiniz.
            </p>
          </div>
        </section>
                ...
                <ul className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                  {gruplar.get(bolge).map((il) => (
                    <li key={il.slug}>
                      ...
                      >
                        <span className="font-semibold text-[#1e3a5f]">{il.ad} Nakliyeciler Sitesi</span>
